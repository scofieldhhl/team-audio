...
public class MultimediaFileScanner {
	...
	private static void initMediaType() {
		...
		// -----------------{"dsf","dff","dst","dsd", "wma", "aif", "aac", "iso"};
		...
		mAudioExts.put("DSF", null);
		mAudioExts.put("DFF", null);
		mAudioExts.put("DST", null);
		mAudioExts.put("DSD", null);
		mAudioExts.put("WMA", null);
		mAudioExts.put("AIF", null);
		mAudioExts.put("AAC", null);
		mAudioExts.put("ISO", null);
		// -----------------
...
//		thread.start();
				...
				if (dirs != null && dirs.length > 0) {
					for (File storage : dirs) {
						if (storage.isDirectory()) {
							...
							long totalSize = (sf.getBlockSize() * sf
									.getBlockCount()) / 1024;
							long availSize = (sf.getBlockSize() * sf
	...
	public static void GetMusicFiles(String Path, boolean IsIterative, List<Song> list)  //搜索目录，扩展名，是否进入子文件夹
	{
							...
							for(String str : arrMusicExtension){
								if(arrFile[length -1].equalsIgnoreCase(str)){
									/*if(Contsant.DSD_ISO.equalsIgnoreCase(str)){
										...
										List<Song> cueMusicList = CueUtil.getMusicsFromIso(cueIso, f.getPath());
